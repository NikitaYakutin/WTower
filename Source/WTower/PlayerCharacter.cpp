    ...
    void APlayerCharacter::AddControllerPitchInput(float Value)
    {
        if (Value != 0.0f && Controller)
        {
            ...
            // Преобразуем угол Pitch для корректной работы с ограничениями
            // Unreal использует диапазон 0-360, но нам удобнее работать с -90 до +90
            float CurrentPitch = CurrentRotation.Pitch;
            if (CurrentPitch > 180.0f)
            {
                CurrentPitch = CurrentPitch - 360.0f; // Преобразуем диапазон [270, 360] в [-90, 0]
            }

            float NewPitch = CurrentPitch + ScaledValue;

            // Ключевое изменение: Всегда разрешаем движение ОБРАТНО от крайних углов
            // Это позволит выбраться из "застрявшего" положения
            bool bAllowMovement = true;

            // Проверяем выход за ограничения только если движение увеличивает отклонение
            if ((NewPitch > MaxPitchAngle && ScaledValue > 0) ||
                (NewPitch < MinPitchAngle && ScaledValue < 0))
            {
                bAllowMovement = false;
            }

            // Если позволено движение, применяем поворот
            if (bAllowMovement)
            {
                Super::AddControllerPitchInput(ScaledValue);
            }
        }
    }
    //----------------------------------------------------------------------------------------
